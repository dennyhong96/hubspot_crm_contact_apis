...
const createContact = async () => {
	try {
		...
		const {
			body: newContact,
		} = await hubspotClient.crm.contacts.basicApi.create(contactInfo);
...
// ------------------------------------- Batch -------------------------------------
const batchArchiveContacts = async () => {
	try {
		// Get some contacts to archive
...
const batchCreateContacts = async () => {
	try {
		...
		const data = await hubspotClient.crm.contacts.batchApi.create({
			inputs: newContactsInfo,
		});
		console.log(data);
	} catch (error) {
		console.error(error);
...
const batchUpdateContacts = async () => {
	try {
		// Gets some contacts to update
		const fiveContactsToUpdate = (await listContacts())
			.map((contact) => ({ id: contact.id }))
			.slice(0, 3);
		...
		const {
			...
			inputs: fiveContactsToUpdate.map((c) => ({
				id: c.id,
				properties: {
					website: "somewebsite.io",
				},
			})),
		});

		console.log(updatedContacts.results.map((c) => c.properties));
	} catch (error) {
		console.error(error);
	}
};
